...
int g_chs2lba(tCHS chs_geometry, tLBA* lba_geometry)
{
    *lba_geometry = (uint32_t)(
            chs_geometry.cylinder * chs_geometry.head * chs_geometry.sector);
    return 0;
}
...
int g_large2lba(tLARGE large_geometry, tLBA* lba_geometry)
{
    *lba_geometry = large_geometry.cylinder * large_geometry.head
            * large_geometry.sector;
    return 0;
}
...
int g_idechs2lba(tIDECHS idechs_geometry, tLBA* lba_geometry)
{
    *lba_geometry = idechs_geometry.cylinder * idechs_geometry.head
            * idechs_geometry.sector;
    return 0;
}
...
{
    *lba = (uint32_t)(
            chs.cylinder * geom.head + chs.head * geom.sector + chs.sector - 1);
    return 0;
}
